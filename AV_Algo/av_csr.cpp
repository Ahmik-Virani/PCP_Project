...
public:
    GM_EdgeLock_CSR(const vector<vector<int>>& adj, int p) : p(p) {
        this->n = adj.size();
        ...
        for (int i = 0; i < p; i++) {
            threads.emplace_back(&GM_EdgeLock_CSR::pseudo_color, this, chunks[i].first, chunks[i].second);
        }
