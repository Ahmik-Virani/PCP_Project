...
private:
    ...
    void run_GM(){
        ...
        for(int i = 0 ; i < p; i++){
            ...
            threads[i] = thread(&GM_EdgeLock_Self::pseudo_color, this, chunks[i].first, chunks[i].second, ref(color));
...
public:
    GM_EdgeLock_Self (vector<vector<int>> adj, int p) {
        n = adj.size();
        this->adj = adj;
        this->p = p;
