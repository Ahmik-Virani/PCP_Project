...
private:
    ...
    void run_GM(){
        ...
        for(int i = 0 ; i < p; i++){
            ...
            threads[i] = thread(&GM_EdgeLock_General::pseudo_color, this, chunks[i].first, chunks[i].second, ref(color));
...
public:
    GM_EdgeLock_General (vector<vector<int>> adj, int p) {
        n = adj.size();
        this->adj = adj;
        this->p = p;
