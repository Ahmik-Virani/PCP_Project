...
private:    
    ...
    void run_GM(){
        ...
        for(int i = 0 ; i < p; i++){
            ...
            threads[i] = thread(&GM_prefetch::pseudo_color, this, ind1, ind2, ref(color));
        ...
        for(int i = 0 ; i < p ; i++){
            ...
            threads[i] = thread(&GM_prefetch::check_clash, this, ind1, ind2, cref(color), ref(is_clashing));
...
public:
    ...
    GM_prefetch(vector<vector<int>> adj, int p){
        n = adj.size();
        this->adj = adj;
        this->p = p;
