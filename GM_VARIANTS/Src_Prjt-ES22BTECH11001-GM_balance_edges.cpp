...
private:    
    ...
    void run_GM(){
        ...
        for(int i = 0 ; i < p; i++){
            ...
            threads[i] = thread(&GM_balance_edges::pseudo_color, this, chunks[i].first, chunks[i].second, ref(color));
        ...
        for(int i = 0 ; i < p ; i++){
            ...
            threads[i] = thread(&GM_balance_edges::check_clash, this, chunks[i].first, chunks[i].second, cref(color), ref(is_clashing));
...
public:
    ...
    GM_balance_edges(vector<vector<int>> adj, int p){
        n = adj.size();
        this->adj = adj;
        this->p = p;
